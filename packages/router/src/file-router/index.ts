...
import type { FileRouterOptions, PageNode } from './types/index.js'
import {
  applyPathStrategy,
  info,
...
export class FileRouter {
   ...
   * @private
   */
  readonly #fileMap: Map<string, PageNode> = new Map()
  /**
   * 生成结果
   * @private
   */
   ...
   * 键为文件或目录路径，值为对应的节点对象
   */
  get fileMap(): Map<string, PageNode> {
    return this.#fileMap
  }
   ...
   * @returns 扫描到的页面文件列表
   */
  protected scanPages(): PageNode[] {
    const pages: PageNode[] = []
    for (const page of this.config.pages) {
      if (page.group && page.prefix) {
        const route: PageNode = {
          filePath: page.dir,
  ...
  private processFile(
    ...
    fileName: string,
    page: ScanDirConfig,
    pageMapping: Map<string, PageNode>,
    parent?: PageNode
  ): PageNode | null {
    // 获取扩展名称
  ...
  public generate(): GenerateResult {
    if (!this.#generateResult) {
      this.#generateResult = generateRoutes(this.pageTree, {
  ...
  public addPage(filePath: string): boolean {
    ...
    if (filename.includes('@')) {
      ...
      const pages = parent ? parent.children! : this.pageTree
      const newRoutePath = this.applyPathStrategy(prefix + baseName)
      let sameRoute: PageNode | null = null
